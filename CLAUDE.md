...
Opt-in autonomous operation. Four first-class modes — `off` (default), `shadow` (logs verdicts only), `park` (decides but never merges; **spawns new work from backlog**), `auto` (merges to per-run staging branch; **spawns new work from backlog**). Backlog top-up runs inside the worker-manager promote tick (~5s) and is gated by `headroom = concurrencyCap − running − pending`. Per-tick spawn limit `autonomy.backlogSpawnPerTick` (default 2). Items are claimed atomically (`consumedBy`) before spawn; mid-spawn crash never double-issues. Opt out via `WORKFORCE_AUTONOMY_SPAWN_BACKLOG=0` or `autonomy.spawnFromBacklog: false`. Halt suspends both spawning and merges. Resolution: `WORKFORCE_AUTONOMY` env (also accepts `halt` as kill switch) → active `autonomy_runs` row for the repo → `autonomy.mode` in `defaults.json` → fallback `off`. The controller (`core/autonomy-controller.js`) owns mode, lease, halt, budget cap, and concurrency override.
