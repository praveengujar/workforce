...
Codex plugin for managing autonomous agent sessions.

## Usage

```bash
Codex --plugin-dir .   # Load this directory as a Codex plugin
```
...
Each task spawns Codex CLI as a child process in an isolated git worktree (`wf/{id}` branch).
...
- Zero-work guard: if Codex made no real code changes, task is marked `failed`
